...
const (
	resumptionActionNone int8 = iota
	resumptionActionAdd
	resumptionActionCheck
)
...
var (
	...
	errReuse                 = errors.New("TLS session reuse error")
)

var (
	resumptionIDPrefix = []byte{0x46, 0x54, 0x50, 0x01} //nolint:gochecknoglobals
)
...
type clientHandler struct {
	...
	server              *Server         // Server on which the connection was accepted
	...
	tlsConfig           *tls.Config     // If set will be used instead of calling driver.GetTLSConfig
	resumptionID        []byte          // ID for session resumption
	resumptionAction    int8            // 0 do nothing, 1 add resumption ID, 2 check resumption ID
...
func (server *Server) newClientHandler(
	connection net.Conn,
	clientID uint32,
...
func (c *clientHandler) getTLSConfig() (*tls.Config, error) {
	if c.server.settings.TLSSessionReuse == TLSSessionReuseRequired {
		c.resumptionAction = resumptionActionNone
		if c.tlsConfig != nil {
...
func (c *clientHandler) addTLSReuseCallbacks(config *tls.Config) {
...
func generateSessionResumptionID() ([]byte, error) {
	b := make([]byte, 32)
