...
// MainDriverExtensionTLSConnectionStateVerifier is an extension that allow to
// pass the tls.ConnectionState from the tls.VerifyConnection. This must be
// implemented if you enable TLS session resumptions and need to check the
// tls.ConnectionState. When enabling TLS session resumption the library will
// set the tls.VerifyConnection callback
type MainDriverExtensionTLSConnectionStateVerifier interface {
	VerifyTLSConnectionState(cc ClientContext, cs tls.ConnectionState) error
}
...
// DirLister defines the interface for listing directory contents.
...
type ClientDriverExtensionFileList interface {
	...
	ReadDir(name string) (DirLister, error)
}
...
// TLSSessionReuse is th enumerable that represents the supported TLS session reuse modes
type TLSSessionReuse int8

// TLS session reuse modes
const (
	// clients may or may not reuse TLS sessions, no check is done
	TLSSessionReuseUnchecked TLSSessionReuse = iota
	// reusing TLS sessions for data connections is required
...
type Settings struct {
	...
	TLSSessionReuse          TLSSessionReuse  // define sthe TLS session reuse mode
	...
	IgnoreASCIITranferType   bool             // Silently ignore ASCII transfer mode http://www.proftpd.org/docs/modules/mod_xfer.html
