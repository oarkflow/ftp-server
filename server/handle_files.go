	...
	if fileList, ok := c.driver.(ClientDriverExtensionFileList); ok {
		...
		for {
			...
			if err != nil && !errors.Is(err, io.EOF) {
				if !replySent {
					c.writeMessage(StatusFileActionNotTaken, fmt.Sprintf("Could not list batch: %v", err))
				} else {
					c.logger.Error("error handling STAT file after sending the initial reply", "err", err)
				}
				return nil
			}
			if !replySent {
