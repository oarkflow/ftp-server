...
// This is shared between Server instances as there's no point in making the FTP commands behave differently
// between them.
...
// Server is where everything is stored
...
func (server *Server) loadSettings() error {
	...
	if settings.TLSRequired == ImplicitEncryption && settings.TLSSessionReuse != TLSSessionReuseUnchecked {
		return fmt.Errorf("%w: TLS session reuse is not support for implicit FTPS", errReuse)
	}
...
func (server *Server) Listen() error {
	...
	server.Logger.Info("Listening...", "address", server.listener.Addr().String())

	return nil
}

func (server *Server) createListener() (net.Listener, error) {
...
func (server *Server) handleAcceptError(err error, tempDelay *time.Duration) (bool, error) {
	...
	// & https://github.com/oarkflow/ftp-server/pull/352#pullrequestreview-1077459896
...
func New(driver MainDriver) *Server {
	return &Server{
		driver: driver,
...
// clientDeparture
func (server *Server) clientDeparture(c *clientHandler) {
